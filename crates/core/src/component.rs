...
impl ComponentKind {
    pub fn impedance(&self, omega: AngularFrequency) -> ImpedanceResult {
        match self {
            Self::Resistor { r } => match Option::<f64>::from(r.clone()) {
                Some(r_val) => ImpedanceResult::new_finite(Complex64::new(r_val, 0.0)),
                None => ImpedanceResult::Open,
            },
            Self::Inductor { l } => match Option::<f64>::from(l.clone()) {
                Some(l_val) => {
                    ImpedanceResult::new_finite(Complex64::new(0.0, f64::from(omega) * l_val))
                }
                None => ImpedanceResult::Open,
            },
            Self::Capacitor { c } => match Option::<f64>::from(c.clone()) {
                Some(c_val) => {
    ...
    match z {
        ImpedanceResult::Finite(z_val) => {
            if z_val.im.abs() < EPSILON {
                ...
                Ok(ComponentKind::Impedance {
                    z: ImpedanceResult::new_finite(z_val),
                })
            }
        }
        ImpedanceResult::Open => Ok(ComponentKind::Impedance {
            z: ImpedanceResult::Open,
        }),
        ImpedanceResult::Short => Ok(ComponentKind::Impedance {
            z: ImpedanceResult::Short,
        }),
    }
}
