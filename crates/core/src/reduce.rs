...
use crate::units::{combine_parallel_many, combine_series, AngularFrequency, ImpedanceResult};
...
) -> Result<Vec<ReductionStep>, CircuitError> {
...
fn find_series_reduction(graph: &CircuitGraph) -> Option<ReductionStep> {
    for node_idx in 0..graph.nodes.len() {
        ...
        let outer1 = if comp1.nodes.0 != middle_idx {
            comp1.nodes.0
        } else {
            comp1.nodes.1
        };
        let outer2 = if comp2.nodes.0 != middle_idx {
            comp2.nodes.0
        } else {
            comp2.nodes.1
        };
...
fn apply_reduction(
    graph: &mut CircuitGraph,
    step: &mut ReductionStep,
) -> Result<ComponentIndex, CircuitError> {
    match step {
        ReductionStep::Series {
            components,
            impedance,
            equivalent,
            nodes,
        }
        | ReductionStep::Parallel {
            components,
            impedance,
            equivalent,
            nodes,
